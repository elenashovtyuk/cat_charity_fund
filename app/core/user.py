...
# добавим асинхронный генератор. Он обеспечит доступ к БД через SQLAlchemy
# В дальнейшем он будет использоваться в качестве зависимости для класса
# UserManager
async def get_user_db(session: AsyncSession = Depends(get_async_session)):
    yield SQLAlchemyUserDatabase(session, User)


# далее добавим компоненты, которые необходимы для построения
# аутентификационного бэкенда - стратегию, транспорт, сам бэкенд
...
auth_backend = AuthenticationBackend(
    name='my_best_jwt',  # Произвольное имя бэкенда (должно быть уникальным).
    transport=bearer_transport,
    get_strategy=get_jwt_strategy,
)


# добавим класс UserManager и корутину, которая возвращает объект этого класса
...
# создаем объект класса FastAPIUsers
# это центральный объект, который связывает  объект класса
# UserManager и бэкенд аутентификации
fastapi_users = FastAPIUsers[User, int](
    get_user_manager,
    [auth_backend],
)

# Это методы класса FastAPIUsers, которые мы будем использовать
# в системе Dependency Injection
# для получения текущего пользователя при выполнении запросов,
# а также для разграничения доступа: некоторые эндпоинты будут доступны
#  только суперюзерам.
